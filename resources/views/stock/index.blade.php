...
<div class="card card-custom">
    ...
    <div class="float-xl-right" style="position: absolute; right:0; top:1rem">
        <a href="{{ route('products.create') }}" class="btn btn-light-primary font-weight-bolder btn-md ">Add To
            Stock</a>
    </div>
    <div class="card-body">
            ...
            <table class="table  table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $stock->product->name ?? "Def" }}</td>

                        <td>
                            @if ($stock->variants)
                            @foreach($stock->variants->att_value as $att_val)
                        ...
                        <td>
                            @if($stock->variants)
                            {{ $stock->variants->price }}
                            @else
                            {{ $stock->product->price ?? "0" }}
                            @endif
                        </td>

                        <td>@if($stock->variants)
                        ...
                        <td data-field="Status" aria-label="4" class="datatable-cell">
                            <span style="width: 126px;">
                                <span
                                    class="label label-inline label-pill {{ $stock->movement === 'push' ? 'label-success' : 'label-danger' }}">
                                    {{ $stock->movement }}
                                </span>
                            </span>
                        </td>
                        ...
                        <td>
                            <div class="btn-group" style="position: relative; left:-10px;">
                                {{-- <a href="#"
                                    ...
                                    data-bs-toggle="modal" data-bs-target="#updateModal" data-id="{{ $stock->id }}">
                                ...
                                <form method="POST" action="#">
                                    @csrf
                                    @method('delete')
                                    <button class="btn  deleteVariant" data-id="{{ $stock->id }}">
                                        <span class="svg-icon svg-icon-md svg-icon-primary">
                                ...
                                @if ($stock->product)
                                <a href="{{ route('showMove', ['id' => $stock->product->id]) }}">
                                    View
                                </a>
                                @endif

                                {{-- @if ($stock->variants)
                                <a href="{{ route('showMove', ['id' => $stock->variants->id]) }}">
                                    Show Varinats
                                </a>
                                @endif --}}
